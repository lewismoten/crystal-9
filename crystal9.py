...
class TinyMoEPolicy(nn.Module):
    ...
    def forward_mixed_int2_suffix(self, token_ids: torch.Tensor) -> torch.Tensor:
        """INT2 tracer: rowwise INT2 routed-expert and output matrices only."""
        positions = torch.arange(token_ids.shape[1], device=token_ids.device).unsqueeze(0)
        hidden = self.embedding(token_ids) + self.position(positions)
        causal = torch.triu(torch.ones(token_ids.shape[1], token_ids.shape[1], device=token_ids.device, dtype=torch.bool), diagonal=1)
        attended, _ = self.attention(hidden, hidden, hidden, attn_mask=causal, key_padding_mask=token_ids.eq(0), need_weights=False)
        ...
        for expert in self.experts:
            first, _, second = expert
            value = F.linear(state, quantize_rows_ste(first.weight, 2), first.bias)
            value = F.silu(value)
            expert_outputs.append(F.linear(value, quantize_rows_ste(second.weight, 2), second.bias))
        all_experts = torch.stack(expert_outputs, dim=1)
        routed = all_experts.gather(1, top_indices.unsqueeze(-1).expand(-1, -1, state.shape[-1]))
        state = state + (routed * top_weights.unsqueeze(-1)).sum(dim=1)
        return F.linear(state, quantize_rows_ste(self.output.weight, 2), self.output.bias)

    def forward_mixed_int2_suffix_group4(self, token_ids: torch.Tensor) -> torch.Tensor:
        """INT2 suffix candidate with independent four-value groups per row."""
        positions = torch.arange(token_ids.shape[1], device=token_ids.device).unsqueeze(0)
        hidden = self.embedding(token_ids) + self.position(positions)
        causal = torch.triu(torch.ones(token_ids.shape[1], token_ids.shape[1], device=token_ids.device, dtype=torch.bool), diagonal=1)
        attended, _ = self.attention(hidden, hidden, hidden, attn_mask=causal, key_padding_mask=token_ids.eq(0), need_weights=False)
        ...
        for expert in self.experts:
            first, _, second = expert
            value = F.linear(state, quantize_row_groups_ste(first.weight, 2, 4), first.bias)
            value = F.silu(value)
            expert_outputs.append(F.linear(value, quantize_row_groups_ste(second.weight, 2, 4), second.bias))
        all_experts = torch.stack(expert_outputs, dim=1)
        routed = all_experts.gather(1, top_indices.unsqueeze(-1).expand(-1, -1, state.shape[-1]))
        state = state + (routed * top_weights.unsqueeze(-1)).sum(dim=1)
        return F.linear(state, quantize_row_groups_ste(self.output.weight, 2, 4), self.output.bias)

    def forward_mixed_int2_suffix_group1(self, token_ids: torch.Tensor) -> torch.Tensor:
        """INT2 suffix candidate with one scale per matrix scalar."""
        positions = torch.arange(token_ids.shape[1], device=token_ids.device).unsqueeze(0)
        hidden = self.embedding(token_ids) + self.position(positions)
        causal = torch.triu(torch.ones(token_ids.shape[1], token_ids.shape[1], device=token_ids.device, dtype=torch.bool), diagonal=1)
        attended, _ = self.attention(hidden, hidden, hidden, attn_mask=causal, key_padding_mask=token_ids.eq(0), need_weights=False)
        ...
        routed = all_experts.gather(1, top_indices.unsqueeze(-1).expand(-1, -1, state.shape[-1]))
        state = state + (routed * top_weights.unsqueeze(-1)).sum(dim=1)
        return F.linear(state, quantize_row_groups_ste(self.output.weight, 2, 1), self.output.bias)
    ...
    def forward_mixed_int3_suffix_output_bias_router_weight_group4_router_bias_expert_biases_input_group2(self, token_ids: torch.Tensor) -> torch.Tensor:
        """Accepted INT3 scope plus two-value groupwise INT3 input tables."""
        positions = torch.arange(token_ids.shape[1], device=token_ids.device).unsqueeze(0)
        hidden = F.embedding(token_ids, quantize_row_groups_ste(self.embedding.weight, 3, 2), padding_idx=0)
        hidden = hidden + F.embedding(positions, quantize_row_groups_ste(self.position.weight, 3, 2))
        causal = torch.triu(torch.ones(token_ids.shape[1], token_ids.shape[1], device=token_ids.device, dtype=torch.bool), diagonal=1)
        attended, _ = self.attention(hidden, hidden, hidden, attn_mask=causal, key_padding_mask=token_ids.eq(0), need_weights=False)
        last = (token_ids.ne(0).sum(dim=1) - 1).clamp(min=0)
        state = self.norm(attended[torch.arange(token_ids.shape[0], device=token_ids.device), last])
...
def materialize_mixed_int2_suffix(source: TinyMoEPolicy) -> TinyMoEPolicy:
    """Materialize the scoped INT2 suffix layout exactly."""
    materialized = TinyMoEPolicy(
        source.embedding.num_embeddings,
        source.embedding.embedding_dim,
        len(source.experts),
    ...
    with torch.no_grad():
        for expert in materialized.experts:
            expert[0].weight.copy_(quantize_rows(expert[0].weight, 2))
            expert[2].weight.copy_(quantize_rows(expert[2].weight, 2))
        materialized.output.weight.copy_(quantize_rows(materialized.output.weight, 2))
    return materialized


def materialize_mixed_int2_suffix_group4(source: TinyMoEPolicy) -> TinyMoEPolicy:
    """Materialize the INT2 suffix with four-value groups per matrix row."""
    materialized = TinyMoEPolicy(
        source.embedding.num_embeddings,
        source.embedding.embedding_dim,
        len(source.experts),
    ...
    with torch.no_grad():
        for expert in materialized.experts:
            expert[0].weight.copy_(quantize_row_groups(expert[0].weight, 2, 4))
            expert[2].weight.copy_(quantize_row_groups(expert[2].weight, 2, 4))
        materialized.output.weight.copy_(quantize_row_groups(materialized.output.weight, 2, 4))
    return materialized


def materialize_mixed_int2_suffix_group1(source: TinyMoEPolicy) -> TinyMoEPolicy:
    """Materialize the INT2 suffix with one scale per matrix scalar."""
    materialized = TinyMoEPolicy(
        source.embedding.num_embeddings,
        source.embedding.embedding_dim,
        len(source.experts),
    ...
    with torch.no_grad():
        for expert in materialized.experts:
            expert[0].weight.copy_(quantize_row_groups(expert[0].weight, 2, 1))
            expert[2].weight.copy_(quantize_row_groups(expert[2].weight, 2, 1))
        materialized.output.weight.copy_(quantize_row_groups(materialized.output.weight, 2, 1))
    return materialized
...
def materialize_mixed_int3_suffix_output_bias_router_weight_group4_router_bias_expert_biases_input_group2(source: TinyMoEPolicy) -> TinyMoEPolicy:
    """Materialize the accepted INT3 scope with two-value groupwise input tables."""
    materialized = materialize_mixed_int3_suffix_output_bias_router_weight_group4_router_bias_expert_biases(source)
    with torch.no_grad():
