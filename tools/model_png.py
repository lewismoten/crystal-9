"""Lossless RGB PNG byte transport for Crystal-9 artifacts."""
...
def encode_rgba_png(payload: bytes) -> bytes:
    """Encode bytes as RGB pixels; compatibility name retained for callers."""
    if not payload:
    ...
    metadata = json.dumps(
        {"format": "crystal-9-rgb-byte-png-v1", "source_bytes": len(payload), "sha256": hashlib.sha256(payload).hexdigest()},
        sort_keys=True,
        separators=(",", ":"),
    ).encode()
    return b"".join(
...
def decode_rgba_png(png: bytes) -> tuple[bytes, dict[str, object]]:
    """Decode the fixed Crystal-9 RGB PNG contract and verify its digest."""
