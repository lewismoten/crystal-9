...
from training import run_mixed_int4_input_attention_k_qat


def test_key_qat_changes_only_key_projection_weight(tmp_path, monkeypatch):
    ...
    report = run_mixed_int4_input_attention_k_qat(
        epochs=1, batch_size=2, source_path=source_path, output_dir=tmp_path / "run", device=torch.device("cpu")
    )

    saved = torch.load(tmp_path / "run" / "artifacts-qat-mixed-int4-row-input-attention-q-v-out-k.pt", weights_only=False)["state_dict"]
    ...
    assert report["layout"] == "mixed-int4-row-input-attention-q-v-out-k"
    assert report["qat_forward"]["legal_histories"] == 3
    assert report["materialized_mixed_int4_row_input_attention_q_v_out_k"]["legal_histories"] == 3
