...
def test_mixed_int3_scalar_input_attention_q_k_group2_v_group2_fake_qat_matches_materialized():
    tokenizer = crystal9.GameTokenizer.from_design_file("design.json")
    source = crystal9.TinyMoEPolicy(tokenizer.vocab_size).eval()
    token_ids = torch.tensor([[1, 4, 7, 0], [1, 3, 8, 11]])

    expected = source.forward_mixed_int3_scalar_input_attention_q_k_group2_v_group2(token_ids)
